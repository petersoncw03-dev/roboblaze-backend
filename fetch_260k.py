...
from database import AsyncSessionLocal, Result, init_db

load_dotenv()
logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("Fetch260k")
...
async def fetch_page_with_mirror_rotation(client, page, end_date_str, attempt=1):
    ...
    # Se a Blaze aceitar o limite na query, melhor, mas a paginação padrão (100) é mais segura contra bloqueios.
...
async def fetch_260k_history():
    ...
    total_saved = 0
    pages_to_fetch = 2600 # 2600 páginas * 100 registros = 260.000 pedras
...
if __name__ == "__main__":
    asyncio.run(fetch_260k_history())
