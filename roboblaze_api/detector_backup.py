...
def check_user_signals(last_rolls):
    ...
    for sess_key, session in list(signal_sessions.items()):
        ...
        # ── WIN com confluência ativa ──
        if hit_win and not aborted:
            msg = (
        ...
        # ── WIN enquanto abortado — ciclo encerrou, mas sem registrar ──
        # Como o bot estava abortado (não enviou sinal), o usuário não lucrou.
        # Por isso, mantemos a sessão para que o próximo sinal continue de onde parou (Gale Global).
        if hit_win and aborted:
            print(
                f"🔕 [Detector] WIN durante ABORT | {chat_id} min_conf={min_conf} / {target_color}. "
                f"Sessão mantida para Gale Global (usuário não pegou a entrada).", flush=True
            )
            continue
        ...
        if curr_count < min_conf:
            if not aborted:
                ...
                print(
                    f"🛑 [Detector] ABORT | {chat_id} min_conf={min_conf}/{target_color} — "
                    f"Confluência {curr_count} < {min_conf}", flush=True,
                )
            continue  # abortado: silêncio

        max_gales_for_session = max(b.get("gales", 2) for b in session.get("bot_details", [{}]))
        if signal_gale >= max_gales_for_session:
            # Atingiu max_gales sem bater WIN
            if not aborted:
                msg = f"❌ <b>RED {target_color.upper()} {get_target_emoji(target_color)}</b>\nLimite de proteção atingido."
                send_telegram_message(chat_id, msg)
                save_signal_history(
